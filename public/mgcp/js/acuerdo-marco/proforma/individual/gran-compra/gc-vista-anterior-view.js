...
export default class GCVistaAnteriorView extends ProformaIndividualView {
    ...
    listarProformas = (puedeVerPrecios, puedeDeshacerCotizaciones) => {
        ...
        const $tableProformas = $('#tableProformas').DataTable({
            ...
            ajax: {
                url: route('mgcp.acuerdo-marco.proformas.individual.gran-compra.vista-anterior.generar-lista-para-datatable'),
                type: "POST",
            ...
            columns: [
                ...
                { data: 'plazo_publicar', searchable: false, className: 'text-center' },
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    createdCell: function (td, cellData, rowData, row, col) {
                        if (rowData.estado == 'PENDIENTE') // || rowData.estado == 'COTIZADA'
                        {
                            ...
                            td.setAttribute('class', 'success entero text-center plazo');
                        } else {
                            td.setAttribute('class', 'entero text-center plazo');
                        }
                        td.setAttribute('data-id', rowData.nro_proforma);
                        td.setAttribute('data-campo', 'plazo_publicar');
                    }
                },
                {
                    targets: 15,
                ...
                },
                {
                    render: function (data, type, row) {
                        /*let botones = `<div class="btn-group" role="group">
                            <button data-toggle="modal" data-target="#modalOfertasOc" data-marca="${row.producto.marca}" data-modelo="${row.producto.modelo}" data-nroparte="${row.producto.part_no}" 
                            title="Ver ofertas de este producto en órdenes de compra públicas" class="btn btn-default btn-xs ver-ofertas-oc"><span class="fa fa-bar-chart-o" aria-hidden="true"></span></button>
                        ...
                        if (row.producto_tiene_historial_actualizaciones == 1) {
                            botones += `data-empresa="${row.id_empresa}" data-producto="${row.producto.id}"`;
                        } else {
                            botones += 'disabled';
                        }
                        botones += ' class="btn btn-default btn-xs historial-actualizaciones"><span class="fa fa-history" aria-hidden="true"></span></button>';
                ...
                {
                    render: function (data, type, row) {
                        return row.cantidad; //'<div>'+row.cantidad+'</div><div>'+row.paquete+'</div>';
                    }, targets: 12
                },
                {
                    render: function (data, type, row) {
