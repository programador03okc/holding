...
class Paquete extends Model
{
    ...
    public static function generarConsultaRequerimientos(Request $filtros)
    {
        ...
        return $resultado->where('proformas_paquete.id_empresa', Auth::user()->id_empresa)->distinct();
    }
    ...
    public static function generarConsultaEnvioDetalle($proforma, $idEmpresa, $nroRequerimientoEntrega)
    {
        ...
        })->where('proforma',$proforma)->where('proformas_paquete.id_empresa', $idEmpresa)->where('nro_requerimiento_entrega', $nroRequerimientoEntrega)->where('seleccionado',true)->select(['proforma_paquete_envio_detalles.*'])->first();
    }
    ...
    public static function generarConsultaProformas($requerimiento)
    {
        return Paquete::join('mgcp_acuerdo_marco.proforma_paquete_productos', 'proforma_paquete_productos.proforma_paquete_id', 'proformas_paquete.id')
            ...
            ->orderBy('proforma')->orderBy('marca')->orderBy('modelo')->orderBy('part_no')->orderBy('id_producto')->orderBy('proformas_paquete.id_empresa');
    }
}
