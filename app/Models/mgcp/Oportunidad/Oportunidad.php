...
class Oportunidad extends Model
{
    ...
    public static function crearCodigo()
    {
        ...
        // $oportunidades = Oportunidad::where('id_empresa', Auth::user()->id_empresa)->select();
        if ($oportunidades->count() > 0) {
            $inicioMes = Carbon::now('America/Lima')->startOfMonth();
            $finMes = Carbon::now('America/Lima')->endOfMonth();
            $cantNegMes = $oportunidades ->where('created_at', '>=', $inicioMes) ->where('created_at', '<=', $finMes)->count();

            // return $abrev . (Carbon::now('America/Lima')->year - 2000) . str_pad((Carbon::now('America/Lima')->month), 2, "0", STR_PAD_LEFT) . str_pad(($cantNegMes + 1), 3, "0", STR_PAD_LEFT);
            return (Carbon::now('America/Lima')->year - 2000) . str_pad((Carbon::now('America/Lima')->month), 2, "0", STR_PAD_LEFT) . str_pad(($cantNegMes + 1), 3, "0", STR_PAD_LEFT);
        } else {
            // return $abrev . (Carbon::now('America/Lima')->year - 2000) . str_pad((Carbon::now('America/Lima')->month), 2, "0", STR_PAD_LEFT) . str_pad((0 + 1), 3, "0", STR_PAD_LEFT);
            return (Carbon::now('America/Lima')->year - 2000) . str_pad((Carbon::now('America/Lima')->month), 2, "0", STR_PAD_LEFT) . str_pad((0 + 1), 3, "0", STR_PAD_LEFT);
        }
    }
    ...
    {
        return $this->belongsTo(User::class, 'id_responsable')->withTrashed();
    }
