...
class ProformaFiltrosHelper
{
    public static function actualizar(Request $request)
    {
        ...
        if ($request->chkTipoCarga == 'on') {
            session(['proformaTipoCarga' => $request->selectTipoCarga]);
        } else {
            $request->session()->forget('proformaTipoCarga');
        }

        if ($request->chkMPG == 'on') {
            session(['proformaMPG' => true]);
        } else {
            $request->session()->forget('proformaMPG');
        }
    }
}
