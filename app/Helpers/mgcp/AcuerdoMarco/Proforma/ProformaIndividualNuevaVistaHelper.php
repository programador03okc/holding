...
/**
 * Helper que genera la vista de proformas individuales, tanto para Compra ordinaria y Gran compra
 */
class ProformaIndividualNuevaVistaHelper
{
    private $request;
    /**
     * Filas que se muestan por defecto en la vista
     */
    ...
    /**
     * Genera la lista de proformas, dependiendo de los filtros aplicados
     */
    public function generarLista(User $usuario)
    {
        ...
        foreach ($requerimientos as $requerimiento) {
            ...
            if ($this->request->tipoProforma == 1) {
                $proformas = CompraOrdinaria::generarConsultaProformas($this->request)->where('requerimiento', $requerimiento->requerimiento)
                ->select([
                    'nro_proforma', 'proforma', 'fecha_emision', 'fecha_limite', 'inicio_entrega', 'fin_entrega', 'marca', 'modelo', 'part_no',
                    'id_entidad', 'proformas_compra_ordinaria.id_empresa', 'id_producto', 'software_educativo', 'cantidad', 'precio_unitario_base', 'moneda_ofertada',
                    'id_ultimo_usuario', 'estado', 'costo_envio_publicar', 'proforma', 'categorias.descripcion AS categoria', 'requiere_flete',
                    'precio_publicar', 'plazo_publicar','users.nombre_corto AS nombre_usuario', 'restringir','puede_restringir', 'probabilidad_ganar'
                ])->where('proformas_compra_ordinaria.id_empresa', Auth::user()->id_empresa)->get();
            } else {
                $proformas = GranCompra::generarConsultaProformas($this->request)->where('requerimiento', $requerimiento->requerimiento)
                ->select([
                    'nro_proforma', 'proforma', 'fecha_emision', 'fecha_limite', 'inicio_entrega', 'fin_entrega', 'marca', 'modelo', 'part_no',
                    'id_entidad', 'proformas_gran_compra.id_empresa', 'id_producto', 'software_educativo', 'cantidad', 'precio_unitario_base', 'moneda_ofertada',
                    'id_ultimo_usuario', 'estado', 'costo_envio_publicar', 'proforma', 'categorias.descripcion AS categoria', 'requiere_flete',
                    'precio_publicar', 'plazo_publicar','users.nombre_corto AS nombre_usuario', 'restringir','puede_restringir', 'probabilidad_ganar'
                ])->where('proformas_gran_compra.id_empresa', Auth::user()->id_empresa)->get();
            }
            $proformaInicial = 'x';
            $productoInicial = 0;
            ...
            foreach ($proformas as $proforma) {
                    ...
                    <td class="text-center">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <!--<li>
                                    <a class="ver-ofertas-oc" data-toggle="modal" data-target="#modalOfertasOc" data-marca="'.$proforma->marca.'" data-modelo="' . $proforma->modelo.'" data-nroparte="' . $proforma->part_no.'" href="#">
                                ...
                                if (Auth::user()->tieneRol(67)) {
                                    $resultado .= '
                                    <li>
                                        <a class="analisis analisis-'.$proforma->proforma.'-'.$proforma->id_producto.'" 
                                            data-producto="'.$proforma->id_producto.'" data-proforma="'.$proforma->proforma.'" data-nro="'.$proforma->nro_proforma.'" data-tipo="'.$this->request->tipoProforma.'" data-cantidad='.$proforma->cantidad.'  href="#">
                                            <span class="glyphicon glyphicon-random"></span>Análisis de resultados
                                        </a>
                                    </li>';
                                }
                                $resultado .= '
                                <li>
                                    <a class="comentarios" data-id="'.$proforma->nro_proforma.'" data-proforma="'.$proforma->proforma.'" data-tipo="'.$this->request->tipoProforma.'" href="#"><span class="glyphicon glyphicon-comment"></span>Comentarios en proforma</a>
                                </li>
                                <li>
                                    <a class="calculadora" data-cantidad='.$proforma->cantidad.' data-nro="'.$proforma->nro_proforma.'" 
                                        data-requerimiento="'.str_replace('<strong style="color:red">(Emergencia COVID-19)</strong>', '', $requerimiento->requerimiento).'"
                                        data-proforma="'.$proforma->proforma.'" data-tipo="'.$this->request->tipoProforma.'" data-producto="'.$proforma->id_producto.'" href="#">
                                        <span class="glyphicon glyphicon-th"></span>Calculadora
                                    </a>
                                </li>
                            </ul>
                        </div>
                        ...
                        <table class="table table-condensed table-bordered ingreso-datos" style="margin-bottom: 0px !important;">
                            <thead>
                                <tr>
                                    ...
                                    <th title="Mayor Probabilidad de Ganar" class="text-center">MPG</th>
                                </tr>
                            </thead>
                            <tbody>';
                }
                ...
                <tr>
                    ...
                    if ($proformas[$contadorFilaIngreso]->estado == 'PENDIENTE') {
                        ...
                        if ($proformas[$contadorFilaIngreso]->puede_deshacer_cotizacion && ($proformas[$contadorFilaIngreso]->id_ultimo_usuario == $usuario->id || $proformas[$contadorFilaIngreso]->id_ultimo_usuario == 24 || $puedeDeshacerTodasLasCotizaciones)) {
                    ...
                    $resultado .='</td><td class="text-center mpg">';
                    if (Auth::user()->tieneRol(67)) {
                        $resultado .= '<input type="checkbox" name="mpg" class="check-mpg mpg-'.$proforma->proforma.'-'.$proforma->id_producto.' 
                        '.($proformas[$contadorFilaIngreso]->probabilidad_ganar ? 'check-activo-'.$proforma->proforma.'-'.$proforma->id_producto : '').'" 
                        '.($proformas[$contadorFilaIngreso]->probabilidad_ganar ? 'checked' : '').'
                        data-proforma="'.$proforma->proforma.'" data-nro="'.$proformas[$contadorFilaIngreso]->nro_proforma.'" data-producto="'.$proforma->id_producto.'">';
                    }
    ...
    /**
     * Genera la paginación de proformas, donde el usuario puede avanzar, retroceder o salta a una página específica
     */
