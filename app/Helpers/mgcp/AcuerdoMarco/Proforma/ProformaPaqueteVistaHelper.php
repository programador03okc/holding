...
class ProformaPaqueteVistaHelper
{
    ...
    {
        $dtsEmpresa = Empresa::find(Auth::user()->id_empresa);
        $estadoRequerimiento = Paquete::where('requerimiento', $requerimiento->requerimiento)->where('id_empresa', $dtsEmpresa->id)->first();
            ...
            if ($lugarEntrega != $destino->lugar_entrega) {
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <table class="table table-bordered table-condensed envio" style="width: 58%; font-size: small; margin: auto; margin-bottom: 10px;">
                            <thead>
                                <tr>
                                    <th rowspan="2" style="width: 35%" class="text-center">Proforma</th>
                                    <th rowspan="2" style="width: 10%" class="text-center">Cant.</th>
                                    <th class="text-center separador-left separador-right">
                                    ' . $dtsEmpresa->semaforo.' '.$dtsEmpresa->empresa.($estadoRequerimiento != null ? '<br><span style="font-weight: normal; font-size: 0.85em">('.$estadoRequerimiento->estado . ')</span>' : '').
                                    '</th>
                                </tr>
                                <tr>
                            </thead>
    ...
    public function generarTablaProductos($requerimiento, $empresas)
    {
        ...
        $resultado = '<div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <table class="table table-bordered table-condensed" style="width: 100%; font-size: small;margin-bottom: 10px;">
                                <thead>
                                    <tr>
                                        <th rowspan="2" style="width: 20%" class="text-center">Proforma</th>
                                        <th rowspan="2" style="width: 7%" class="text-center">Cant.</th>
                                        <th rowspan="2" class="text-center">Producto</th>
                                        <th rowspan="2" style="width: 8%" class="text-center">Herr.</th>
                                        <th rowspan="2" colspan="2" class="text-center separador-left separador-right">
                                            ' . $dtsEmpresa->semaforo . ' ' . $dtsEmpresa->empresa . ($estadoRequerimiento != null ? '<br><span style="font-weight: normal; font-size: 0.85em">(' . $estadoRequerimiento->estado . ')</span>' : '') .
                                        '</th>
                                    </tr>
                                </thead>
        ...
        $proformas = Paquete::generarConsultaProformas($requerimiento->requerimiento)->select([
            'requerimiento', 'proforma', 'marca', 'modelo', 'part_no', 'id_producto', 'proformas_paquete.id_empresa', 'categorias.descripcion AS categoria', 'cantidad', 'estado',
                    ...
                    if ($productoActual != $proformas[$i]->id_producto) {
                        ...
                        foreach ($empresas as $empresa) {
                            ...
                            if ($posEncontrado >= 0) {
                                ...
                                $resultado .= '<td class="text-center separador-left separador-right">-</td>';
                            }
                            $resultado .= '</td>';
                        }
                        $resultado .=   '</tr>';
