...
class ListaCuadroCostoExport
{
    ...
    private function generarContenido($data)
    {
        ...
        foreach ($data as $cuadro) {
            ...
            $detalleFila = CuadroCostoHelper::obtenerDetallesFilas($cuadro->id);
            $montoTotal = $detalleFila->cuadroAm->monto_adjudicado_mas_igv_format;
            $this->hoja->setCellValue("N$fila", $montoTotal);
            $this->hoja->setCellValue("O$fila", $cuadro->estado_aprobacion);
            $this->hoja->setCellValue("P$fila", $cuadro->responsable_aprobacion);
            $fila++;
        }
    }
    ...
    private function darFormatoContenido($data)
    {
        ...
        $this->hoja->getStyle("N$filaInicial:N$totalFilas")->getNumberFormat()->setFormatCode('"S/"#,##0.00_-');
    }
    ...
    private function generarCabecera()
    {
        ...
        $this->hoja->setCellValue("N$fila", "Total");
        $this->hoja->setCellValue("O$fila", "Estado de aprobación");
        $this->hoja->setCellValue("P$fila", "Responsable aprobación");
        ...
        $this->hoja->getStyle("A$fila:P$fila")->applyFromArray($styleArray)->getAlignment()->setWrapText(true);
    }
    ...
    private function establecerAnchoColumnas()
    {
        ...
        $this->hoja->getColumnDimension('N')->setWidth(19);
        $this->hoja->getColumnDimension('O')->setWidth(29);
        $this->hoja->getColumnDimension('P')->setWidth(21);
    }
}
