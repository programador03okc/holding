        ...
        <div class="row">
            <div class="col-sm-6 col-sm-offset-3">
                <table id="tableFueraMonto" class="table table-condensed table-bordered" style="font-size: small; width: 100%">
                    ...
                    <tbody>
                        <tr>
                            <td id="tdFueraMontoUsuario">{{$aprobadorFueraMonto->usuario->name}}</td>
                            <td class="text-center">
                                <button id="btnEditarFueraMonto" title="Editar" class="btn btn-primary editar btn-xs"
                                    data-id="{{ $aprobadorFueraMonto->id }}" data-usuario="{{ $aprobadorFueraMonto->usuario->id }}">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="hidden" id="idFueraMonto">
    ...
    <script>
        $(document).ready(function () {
            ...
            const $tablePorMonto = $('#tablePorMonto').DataTable({
                ...
                order: [[1, "asc"]],
                ajax: {
                    url: "{{ route('mgcp.cuadro-costos.ajustes.aprobadores.data-lista') }}",
                    type: "POST",
            ...
            $('#tablePorMonto').on('click', 'button.eliminar', function () {
                if (confirm("¿Está seguro de eliminar el registro?")) {
                    var $boton = $(this);
                    $.ajax({
                        url: '{{route("mgcp.cuadro-costos.ajustes.aprobadores.eliminar-aprobador-por-monto")}}',
            ...
            $('#btnEditarFueraMonto').click(function () {
                $('#selectAprobadoresFueraMonto').val($(this).data('usuario'));
                $('#idFueraMonto').val($(this).data('id'));
            ...
            $('#btnActualizarAprobadorFueraMonto').click(function () {
                ...
                $.ajax({
                    url: '{{ route("mgcp.cuadro-costos.ajustes.aprobadores.actualizar-aprobador-fuera-monto") }}',
                    data: { id: $('#idFueraMonto').val(), usuario: $('#selectAprobadoresFueraMonto').val(), _token: '{{csrf_token()}}'},
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
                        alert(data.mensaje);
            ...
            $('#tablePorMonto').on('click', 'button.editar', function () {
                ...
                $.ajax({
                    url: '{{ route("mgcp.cuadro-costos.ajustes.aprobadores.detalles-aprobador-por-monto") }}',
                    data: { id: $boton.data('id'), _token: '{{csrf_token()}}' },
                    type: 'POST',
                    dataType: 'json',
                    success: function (data) {
            ...
            $('#btnGuardarPorMonto').click(function () {
                var $boton = $(this);
                $boton.prop('disabled', true);
                var rutaRegistrar = '{{route("mgcp.cuadro-costos.ajustes.aprobadores.registrar-aprobador-por-monto")}}';
                var rutaActualizar = '{{route("mgcp.cuadro-costos.ajustes.aprobadores.actualizar-aprobador-por-monto")}}';
                $.ajax({
